...
function onVersionSelect(pre, version) {
	...
	for (const match of matches) {
		...
		if (type === "modrinth") {
			fetch(`https://api.modrinth.com/v2/project/${extra}/version?game_versions=[%22${version}%22]`).then((response) => {
			...
			params.append("url", `https://ldtteam.jfrog.io/ui/api/v1/download?repoKey=parchmentmc-internal&path=org%2Fparchmentmc%2Fdata%2Fparchment-${version}%2Fmaven-metadata.xml`);
			fetch(`https://corsproxy.io/?${params.toString()}`).then((response => { 
			...
			fetch("https://maven.neoforged.net/releases/net/neoforged/neoforge/maven-metadata.xml").then((response) => {
				response.text().then((data) => {
					const parser = new DOMParser();
					const xml = parser.parseFromString(data, "text/xml");
					const versions = Array.from(xml.querySelectorAll("version"));
					pre.innerText = pre.innerText.replace(full, versions.map((v) => v.textContent).filter((v) => v.startsWith(version.substring(2))).sort().reverse()[0]);
				}).catch(onError);
			}).catch(onError);
		} else if (type === "fabric-loader") {
			fetch(`https://meta.fabricmc.net/v2/versions/loader/${version}`).then((response) => {
				response.json().then((data) => {
					pre.innerText = pre.innerText.replace(full, data[0].loader.version);
				}).catch(onError);
			}).catch(onError);
		} else if (type === "forge") {
			fetch("https://maven.minecraftforge.net/releases/net/minecraftforge/forge/maven-metadata.xml").then((response) => {
				response.text().then((data) => {
					const parser = new DOMParser();
					const xml = parser.parseFromString(data, "text/xml");
					const versions = Array.from(xml.querySelectorAll("version"));
					pre.innerText = pre.innerText.replace(full, versions.map((v) => v.textContent).filter((v) => v.startsWith(version)).sort().reverse()[0]);
				}).catch(onError);
			}).catch(onError);
		}
	}
...
document.addEventListener("DOMContentLoaded", () => {
	...
	fetch("https://meta.fabricmc.net/v2/versions/game").then((response) => {
		response.json().then((data) => {
